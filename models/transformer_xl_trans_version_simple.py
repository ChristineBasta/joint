...
@register_model('transformer_xl_attention_simple')
...
class TranformerXLDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out, incremental_state=None,*mems):
       ...
       #28 May (Christine), the shape may need adaptation
        ...
        k_src_trg_len = srclen + (qlen) + mlen
        ...
        for i, layer in enumerate(self.layers):
            ...
            #4
            process_prev_trg=False
            source_mem_mask = source.new_zeros(srclen+qlen, srclen + mlen).byte()[:, :, None]
            dec_attn_mem_mask = torch.triu(x.new_ones(srclen+qlen, qlen), diagonal=1).byte()[:, :, None]
            dec_attn_mem_mask = torch.cat([source_mem_mask, dec_attn_mem_mask], 1)

            if (process_prev_trg):
                dec_attn_mask = torch.triu(x.new_ones(srclen+qlen, qlen), diagonal=1).byte()[:, :, None]
                source_mask = source.new_zeros(srclen+qlen, srclen).byte()[:, :, None]
            ...
            dec_attn_mem_mask_2 = torch.triu(x.new_ones(srclen+qlen, qlen), diagonal=1).byte()[:, :]
            dec_attn_mem_mask_2= torch.cat([source_mem_mask_2, dec_attn_mem_mask_2], 1)

            if (process_prev_trg):
                dec_attn_mask_2 = torch.triu(x.new_ones((qlen), qlen), diagonal=1).byte()[:, :]
                source_mask_2 = source.new_zeros(qlen, srclen).byte()[:, :]
                dec_attn_mask_2 = torch.cat([source_mask_2, dec_attn_mask_2], 1)
                dec_attn_mem_mask_2 = torch.cat([dec_attn_mask_2, dec_attn_mem_mask_2], 1)
...
@register_model_architecture('transformer_xl_attention_simple', 'transformer_xl_attention_simple')
...
@register_model_architecture('transformer_xl_attention_simple', 'transformer_xl_attention_iwslt_de_en_simple')
...
@register_model_architecture('transformer_xl_attention_simple', 'local_transformer_xl_attention_iwslt_de_en_simple')
...
@register_model_architecture('transformer_xl_attention_simple', 'transformer_xl_attention_wmt_en_de_simple')
def transformer_xl_attention_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('transformer_xl_attention_simple', 'transformer_xl_attention_wmt_en_de_big_simple')
...
@register_model_architecture('transformer_xl_attention_simple', 'local_transformer_xl_attention_wmt_en_de_big_simple')
...
@register_model_architecture('transformer_xl_attention_simple', 'transformer_xl_attention_wmt_en_fr_big_simple')
def transformer_xl_attention_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_xl_attention_wmt_en_de_big(args)


@register_model_architecture('transformer_xl_attention_simple', 'local_transformer_xl_attention_wmt_en_fr_big_simple')
