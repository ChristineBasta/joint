...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
        ...
        self.d_head = d_head
        self.dropout = dropout
        # how do I specify such dimensions??
        # on what basis I choose them?
        self.q_net = nn.Linear(d_model, n_head * d_head, bias=False)
        self.kv_net = nn.Linear(d_model, 2 * n_head * d_head, bias=False)

        self.drop = nn.Dropout(dropout)
        self.dropatt = nn.Dropout(dropatt)
        # it will take from the q_net
    ...
    def forward(self, h, attn_mask=None, mems=None):
        ...
        if mems is not None:
            # the mems is concatenated to the hidden
        ...
        head_q = self.q_net(h)
        # chunk split kvnet in two tensors, head_h, head_k
        head_k, head_v = torch.chunk(self.kv_net(c), 2, -1)

        # changing dimensions, buts till how do we choose the dimensions we change to?
        head_q = head_q.view(h.size(0), h.size(1), self.n_head, self.d_head)
        head_k = head_k.view(c.size(0), c.size(1), self.n_head, self.d_head)
        head_v = head_v.view(c.size(0), c.size(1), self.n_head, self.d_head)

        # [qlen x klen x bsz x n_head]
        # Compute the steps of scaled dit product attention
        # qlen=i
        # klen=j
        # bsz= b
        # n_head=n
        ...
        # matmul of all the previous with V
...
# RelLearnableMultiHeadAttn corresponds to the "relative positional encoding" Shaw et al. (2018) proposed,
# which merges the multiplication W^kR into a single trainable matrix hat{R} (see the last paragraph of Page 5).
...
# The div_val is the ratio used to reduce the embedding dimension from each bin,
# where cutoff is the boundary of the bins.
